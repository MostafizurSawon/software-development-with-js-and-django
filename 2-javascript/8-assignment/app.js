...
const displayData = (data) => {
  ...
  if (!data.length) {
    mainContainer.innerHTML = `
    <div class="mx-auto">
    <img src="./icon.png" class="w-100 mt-4" />
  ...
  data.forEach((data) => {
    // console.log(data);
    // const icon =
    //   '<img class="w-25" src="https://w7.pngwing.com/pngs/15/560/png-transparent-verified-badge-symbol-computer-icons-twitter-discord-flat-icon-blue-text-logo-thumbnail.png" />';
...
const loadData2 = () => {
  fetch(`https://openapi.programming-hero.com/api/videos/category/${curId}`)
    .then((res) => res.json())
    // .then((data) => console.log(curId, data.data));
...
const sortView = (view) => {
  ...
  document.getElementById("sort-views").addEventListener("click", () => {
    const sortedView = view?.sort(
      // (a1, a2) =>
      // console.log(parseFloat(a2.others.views) - parseFloat(a1.others.views))
